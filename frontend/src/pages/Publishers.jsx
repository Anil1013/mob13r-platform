...
export default function Publishers() {
  ...
  return (
    <>
      <Navbar/>
      {toast&&<div style={{position:"fixed",top:80,right:24,zIndex:9999,background:toast.type==="error"?"rgba(239,68,68,0.15)":"rgba(34,197,94,0.15)",border:`1px solid ${toast.type==="error"?"rgba(239,68,68,0.3)":"rgba(34,197,94,0.3)"}`,color:toast.type==="error"?"#ef4444":"#22c55e",padding:"12px 20px",borderRadius:12,fontSize:13}}>{toast.msg}</div>}
      <div style={page}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:32,flexWrap:"wrap",gap:16}}>
          <div><h1 style={{fontFamily:"Syne,sans-serif",fontSize:28,fontWeight:700,color:"#f1f5f9"}}>Publishers</h1><p style={{color:"#475569",fontSize:13,marginTop:4}}>{publishers.length} publishers</p></div>
                  ...
                  <tr key={p.id}>
                    ...
                    <td style={td}>
                      <div style={{display:"flex",alignItems:"center",gap:8}}>
                        ...
                        <button style={{background:"rgba(255,255,255,0.05)",border:"1px solid rgba(255,255,255,0.1)",color:"#94a3b8",padding:"4px 8px",borderRadius:6,cursor:"pointer",fontSize:12}} onClick={()=>setVisibleKeys(v=>({...v,[p.id]:!v[p.id]}))}>👁</button>
                        <button style={{background:"rgba(255,255,255,0.05)",border:"1px solid rgba(255,255,255,0.1)",color:"#94a3b8",padding:"4px 8px",borderRadius:6,cursor:"pointer",fontSize:12}} onClick={()=>copyKey(p.api_key)}>📋</button>
                      </div>
                    </td>
