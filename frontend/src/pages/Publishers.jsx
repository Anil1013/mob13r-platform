...
export default function Publishers() {
  ...
  const add=async()=>{
    ...
    setLoading(true);
    const res=await fetch(`${API_BASE}/api/publishers`,{method:"POST",headers:{"Content-Type":"application/json",Authorization:`Bearer ${token}`},body:JSON.stringify({name, email})});
    const data=await res.json();
              ...
              <tbody>
                {filteredPubs.map(p=>(
                  <tr key={p.id}>
                    ...
                    <td style={{...td,fontSize:12,color:"#64748b"}}>{p.email || <span style={{color:"#cbd5e1"}}>—</span>}</td>
                    <td style={td}>
