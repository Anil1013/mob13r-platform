...
export default function Dashboard() {
  ...
  const total = useMemo(
    () =>
      data.reduce(
        (acc, row) => {
          acc.pin_req += toNumber(row.pin_req);
          acc.unique_req += toNumber(row.unique_req);
          acc.pin_sent += toNumber(row.pin_sent);
          acc.unique_sent += toNumber(row.unique_sent);
          acc.verify_req += toNumber(row.verify_req);
          acc.unique_verify += toNumber(row.unique_verify);
          acc.verified += toNumber(row.verified);
          acc.publisher_verified += toNumber(row.publisher_verified);
          acc.revenue += toNumber(row.revenue);
          acc.publisher_revenue += toNumber(row.publisher_revenue);
            ...
            <tbody>
              {data.map((row, i) => (
                <tr key={`${row.offer_id || "offer"}-${row.publisher_id || "pub"}-${i}`}>
                  ...
                  <td>{money(row.publisher_revenue)}</td>
                  <td style={{ color: toNumber(row.profit) >= 0 ? "green" : "red" }}>
                    {money(row.profit)}
                  </td>
              ...
              <tr>
                ...
                <td>{total.publisher_verified}</td>
                <td>-</td>
                <td>-</td>
                <td>{money(total.revenue)}</td>
                <td>{money(total.publisher_revenue)}</td>
                <td style={{ color: total.profit >= 0 ? "green" : "red" }}>
                  {money(total.profit)}
                </td>
                <td colSpan="3" />
              </tr>
