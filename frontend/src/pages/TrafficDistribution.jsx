...
function humanizeRuleError(code) {
  if (!code) return "Failed to save rule";
  switch (code) {
    case "duplicate_rule":
      return "Rule already exists for same Offer + GEO + Carrier.";
    case "weight_exceeded":
      return "Total weight cannot exceed 100% for this tracking link.";
    case "missing_fields":
      return "Missing required fields (pub_id / tracking_link_id / offer_id).";
    default:
      return code;
  }
}
...
export default function TrafficDistribution() {
  ...
  const loadOffers = async () => {
    ...
    setOffers([]);
    try {
      // Backend: /api/distribution/offers  -> { success:true, offers:[...] }
  ...
  const refreshAllForLink = async (link) => {
    ...
    // Offers are global active offers; no need to filter by GEO/carrier here
    loadOffers();
  };
  ...
  const saveRule = async (form) => {
    ...
    try {
      ...
      if (editingRule) {
        ...
        if (res.data?.success === false) {
          toast.error(humanizeRuleError(res.data.error));
          setSavingRule(false);
          return;
        }
