...
export default function FraudAnalytics() {
  ...
  useEffect(() => {
    fetchAlerts();
    const iv = setInterval(fetchAlerts, 20000); // refresh every 20s
  ...
  // Prepare chart data (alerts per day)
    ...
    <div className="p-6">
      ...
      <div className="flex gap-3 items-center mb-4">
        ...
        <div className="ml-auto flex gap-2">
          <button onClick={() => exportCSV("csv")} className="bg-gray-800 text-white px-3 py-2 rounded">
            Export CSV
          </button>
          <button onClick={() => exportCSV("xlsx")} className="bg-gray-800 text-white px-3 py-2 rounded">
            Export XLSX
          </button>
        </div>
      </div>
