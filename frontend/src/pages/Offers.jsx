      ...
      <div style={styles.page}>
        <div style={styles.container}>
          ...
          <form style={styles.topBar} onSubmit={async (e) => {
            ...
            <select style={styles.select} onChange={e => setOfferForm({...offerForm, advertiser_id: e.target.value})}>
              <option value="">Select Advertiser</option>
              {advertisers.map(a => <option key={a.id} value={a.id}>{a.name}</option>)}
            </select>
            <input placeholder="Service Name" style={styles.input} onChange={e => setOfferForm({...offerForm, service_name: e.target.value})} />
            <input placeholder="CPA" style={styles.inputSmall} onChange={e => setOfferForm({...offerForm, cpa: e.target.value})} />
            <input placeholder="Geo" style={styles.inputSmall} onChange={e => setOfferForm({...offerForm, geo: e.target.value})} />
            <input placeholder="Carrier" style={styles.inputSmall} onChange={e => setOfferForm({...offerForm, carrier: e.target.value})} />
          ...
          <div style={styles.tableWrapper}>
            <table style={styles.mainTable}>
              <thead>
                <tr>
                  <th>ID</th><th>Advertiser</th><th>Service Name</th><th>Geo</th><th>Carrier</th><th>CPA</th><th>Cap</th><th>Used</th><th>Remain</th><th>Revenue</th><th>Control</th>
                </tr>
              </thead>
              <tbody>
                {offers.map(o => (
                  <tr key={o.id}>
                    <td>{o.id}</td>
                    <td style={{fontWeight: '700'}}>{o.advertiser_name}</td>
                    <td><input style={styles.inlineInput} defaultValue={o.service_name} onBlur={e => updateOffer(o.id, {service_name: e.target.value})} /></td>
                    <td><input style={styles.inlineInput} defaultValue={o.geo} onBlur={e => updateOffer(o.id, {geo: e.target.value})} /></td>
                    <td><input style={styles.inlineInput} defaultValue={o.carrier} onBlur={e => updateOffer(o.id, {carrier: e.target.value})} /></td>
                    <td style={{color: '#16a34a', fontWeight: '700'}}>${o.cpa}</td>
                    <td><input style={styles.inlineInput} defaultValue={o.daily_cap} placeholder="∞" onBlur={e => updateOffer(o.id, {daily_cap: e.target.value})} /></td>
                    <td style={{fontWeight: '700'}}>{o.today_hits}</td>
                    <td style={{color: '#ef4444'}}>{remaining(o)}</td>
                    <td style={{color: '#3b82f6', fontWeight: '700'}}>{revenue(o)}</td>
                    <td>
          ...
          {selectedOffer && (
            <div style={styles.manageCard}>
              ...
              <div style={styles.manualBox}>
                <h4 style={{marginTop: 0}}>+ Add New Parameter Key</h4>
                ...
                {parameters.map(p => (
                  <div key={p.id} style={styles.paramCard}>
                    ...
                    <input 
                      ...
                      placeholder={p.param_key.includes('msisdn') ? '{msisdn}' : 'Enter value...'}
...
const styles = {
  page: { background: "#f8fafc", minHeight: "100vh", paddingTop: "60px" },
  ...
  mainTable: { width: "100%", borderCollapse: "collapse", textAlign: "center", fontSize: "13px" },
  inlineInput: { border: 'none', background: 'transparent', textAlign: 'center', width: '90%', fontSize: '13px', outline: 'none' },
