...
export default function DynamicLanding() {
  ...
  const sendPin = async () => {
    if (!msisdn) return alert("Please enter mobile number");
    setLoading(true);
    try {
      const params = new URLSearchParams({
        ...
        msisdn: msisdn,
        geo: landing.geo || 'N/A',
        carrier: landing.carrier || 'N/A',
        'x-api-key': landing.api_key
      });
      
      const res = await fetch(`${API_BASE}/api/publisher/pin/send?${params}`);
  ...
  const verifyPin = async () => {
    if (!otp) return alert("Please enter PIN");
    setLoading(true);
    try {
      const params = new URLSearchParams({
      ...
      const res = await fetch(`${API_BASE}/api/publisher/pin/verify?${params}`);
      const data = await res.json();
    ...
    <div style={styles.container}>
      <div style={styles.card}>
        ...
        <div style={{padding: "20px"}}>
          {step === "msisdn" ? (
            <>
              <input style={styles.input} type="tel" placeholder="Mobile Number" value={msisdn} onChange={(e) => setMsisdn(e.target.value)} />
              <button style={styles.button} onClick={sendPin} disabled={loading}>{loading ? "Sending..." : landing.button_text || "Continue"}</button>
            </>
          ) : (
            <>
              <input style={styles.input} type="number" placeholder="Enter PIN" value={otp} onChange={(e) => setOtp(e.target.value)} />
              <button style={styles.button} onClick={verifyPin} disabled={loading}>{loading ? "Verifying..." : "Confirm"}</button>
            </>
          )}
        </div>
