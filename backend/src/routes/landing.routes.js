...
router.post("/", async (req, res) => {
  try {
    ...
    /* ================= SAFE FILE HANDLING (PROMISE WRAPPER) ================= */
    // ✅ This block prevents 'Unexpected end of form' by awaiting the stream fully
    if (req.files && req.files.imageFile) {
      ...
      if (file && file.name && file.size > 0) {
        ...
        // ✅ Using manual promise to ensure the file system is ready
        await new Promise((resolve, reject) => {
          file.mv(savePath, (err) => {
            if (err) {
              console.error("MV Move Error:", err);
              reject(err);
            } else {
              resolve();
            }
    ...
    // ✅ Return FAILED instead of letting Express send a 500 status crash
    res.json({
      status: "FAILED",
      error: err.message,
    });
