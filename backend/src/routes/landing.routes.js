...
router.post("/", async (req, res) => {
  try {
    ...
    // ✅ FIXED MULTIPART HANDLING: Syncing with app.js useTempFiles
    if (req.files && req.files.imageFile) {
      ...
      // 🔥 mv function properly handle karega kyunki server.js me useTempFiles true hai
      await file.mv(savePath);
      finalImage = `/uploads/landings/${fileName}`;
    }

    /* 🔥 INSERT LOGIC (Exact as your old code) */
    ...
    res.json({ status: "FAILED", error: err.message }); // ❌ Prevents 500 error status
  }
});

/* 🔥 GET ALL LANDINGS (No logic changed) */
