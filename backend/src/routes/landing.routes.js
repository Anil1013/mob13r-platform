...
router.get("/publisher-offers", async (req, res) => {
  try {
    const result = await pool.query(`SELECT po.id, o.service_name, p.name AS publisher_name FROM publisher_offers po JOIN offers o ON o.id = po.offer_id JOIN publishers p ON p.id = po.publisher_id`);
    res.json({ status: "SUCCESS", data: result.rows });
  } catch (err) { res.json({ status: "FAILED", error: err.message }); }
});
...
router.post("/", async (req, res) => {
  try {
    ...
    if (req.files && req.files.imageFile) {
      ...
      finalImg = `/uploads/landings/${fName}`; // ✅ Path fix for static serving
    }
...
router.get("/:id", async (req, res) => {
  try {
    const result = await pool.query(`SELECT lp.*, po.offer_id, p.api_key, o.redirect_url, o.geo, o.carrier FROM landing_pages lp JOIN publisher_offers po ON po.id = lp.publisher_offer_id JOIN publishers p ON p.id = po.publisher_id JOIN offers o ON o.id = po.offer_id WHERE lp.id = $1`, [req.params.id]);
