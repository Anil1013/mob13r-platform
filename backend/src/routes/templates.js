...
router.get("/", authJWT, async (req, res) => {
  try {
    const { rows } = await pool.query(
      "SELECT * FROM offer_templates ORDER BY id DESC"
    );
    res.json(rows);
  } catch (err) {
...
router.post("/", authJWT, async (req, res) => {
  try {
    const {
      ...
      pin_send_url,
      pin_verify_url,
      check_status_url,   // ✅ FIXED (matches DB)
      portal_url,
      parameters,
      description,
    } = req.body;
    ...
    const query = `
      ...
      (template_name, country_code, carrier, api_type, pin_send_url, pin_verify_url,
       check_status_url, portal_url, parameters, description)
    ...
    const values = [
      ...
      pin_send_url,
      pin_verify_url,
      check_status_url,                  // ✅ saving correctly
      portal_url,
...
router.put("/:id", authJWT, async (req, res) => {
  try {
    const {
      ...
      pin_send_url,
      pin_verify_url,
      check_status_url,   // ❗ must match DB
      portal_url,
      parameters,
      description,
    } = req.body;
    ...
    const query = `
      ...
      SET template_name=$1, country_code=$2, carrier=$3, api_type=$4,
          pin_send_url=$5, pin_verify_url=$6, check_status_url=$7,
    ...
    const values = [
      ...
      pin_send_url,
      pin_verify_url,
      check_status_url,             // 🔥 Correct column
      portal_url,
      parameters ? JSON.stringify(parameters) : null,
      description,
      req.params.id,
...
router.delete("/:id", authJWT, async (req, res) => {
  try {
    await pool.query("DELETE FROM offer_templates WHERE id=$1", [
      req.params.id,
    ]);
