...
router.get("/", authJWT, async (req, res) => {
  try {
    const q = await pool.query(`
      SELECT o.*, a.name AS advertiser_name, t.template_name
      FROM offers o
...
router.get("/advertisers", authJWT, async (req, res) => {
  try {
    const q = await pool.query("SELECT id, name FROM advertisers ORDER BY name ASC");
    res.json(q.rows);
  } catch (err) {
...
router.post("/", authJWT, async (req, res) => {
  try {
    ...
    const adv = await pool.query("SELECT id FROM advertisers WHERE name=$1", [advertiser_name]);
    if (adv.rows.length === 0) return res.status(400).json({ error: "Advertiser not found" });
    const advertiser_id = adv.rows[0].id;
    ...
    const q = await pool.query(
      `INSERT INTO offers (
         offer_id, advertiser_id, name, type, payout, tracking_url,
...
router.put("/:offer_id", authJWT, async (req, res) => {
  try {
    ...
    const q = await pool.query(
      `UPDATE offers
       SET advertiser_id=$1, name=$2, type=$3, payout=$4, tracking_url=$5,
      ...
      [
        advertiser_id, name, type, payout, tracking_url, cap_daily, cap_total,
