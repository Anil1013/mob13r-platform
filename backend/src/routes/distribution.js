...
/**
 * Smart scoring for rule selection
 */
function scoreRule(rule, reqGeo, reqCarrier) {
  ...
  return { score, weight: Number(rule.weight) || 0 };
}
...
router.get("/meta", async (req, res) => {
  try {
    ...
    const { rows } = await pool.query(q, [pub_id.toUpperCase()]);
    res.json(rows);
  } catch (err) {
...
router.get("/click", fraudCheck, async (req, res) => {
  try {
    ...
    if (!bestRule) {
      bestRule = rules.reduce((max, r) => {
        const w = Number(r.weight) || 0;
        return !max || w > (Number(max.weight) || 0) ? r : max;
      }, null);
    }
...
/* ===========================================================
   REMAINING RULES — FIXED VERSION (NO daily_cap ERROR)
   =========================================================== */
router.get("/rules/remaining", async (req, res) => {
  try {
    ...
    // SAFE QUERY — NO daily_cap column issue
    const q = `
      SELECT 
        id,
        pub_id,
